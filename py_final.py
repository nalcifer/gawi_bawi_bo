...
def joueurVSbot ():
    ...
    continuer = True
    while (continuer):
        bot = options[randint(0,2)] #choisir au hasard le choix du bot
...
def joueurVSjoueur () :
    ...
    while (continuer):
        ...
        if (joueur1.upper()== 'FIN') :
            continuer = False
        if (joueur1.upper()!= 'FIN') :
            joueur2 = input(namejoueur2 +" choisissez : Pierre, Feuille ou Ciseaux? ou tapez Fin pour arrêter le jeu!\n")
        if (joueur2.upper()== 'FIN') :
            continuer = False
        elif (joueur1.upper() == 'PUIT' or joueur2.upper() == 'PUIT') :
            print ("On a dit pierre, feuille ou ciseaux.")
        elif(joueur1.upper() == joueur2.upper()):
            print("Joueur1 et joueur2 ont joué", joueur1.upper(), ".Egalité!")
            tour=tour+1
            print("Tour n°",tour,"fini\n")
